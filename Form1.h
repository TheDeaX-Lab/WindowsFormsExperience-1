...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label1->Text = L"Íîìåð êâàðòèðû:";
			// 
			// comboBox2
			// 
	...
	private: System::Void îòêðûòüToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		case System::Windows::Forms::DialogResult::OK:
		{
			...
			try {
				...
				System::Windows::Forms::MessageBox::Show("Íåâåðíûé ôîðìàò äàííûõ!", "Îøèáêà", System::Windows::Forms::MessageBoxButtons::OK, System::Windows::Forms::MessageBoxIcon::Error);
			}
			fin->close();
			delete fin;
		}
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (comboBox1->SelectedIndex == -1) {
			this->errorProvider2->SetError(comboBox1, "Âû äîëæíû íîìåð êâàðòèðû äëÿ ðåãèñòðàöèè!");
			valid = false;
		}
		else {
		...
		if (!this->datetime_checked) {
			this->errorProvider2->SetError(this->dateTimePicker1, "Âû ïðîèãíîðèðîâàëè íåâåðíóþ äàòó ðîæäåíèÿ!");
			valid = false;
		}
		fio = this->textBox8->Text;
		if (System::String::IsNullOrEmpty(fio)) {
			this->errorProvider2->SetError(this->textBox8, "Âû äîëæíû ââåñòè ÔÈÎ!");
			valid = false;
		}
		if (this->groupBox1->Visible) {
			if (this->radioButton1->Checked) {
				...
				this->errorProvider2->SetError(this->groupBox1, "Âû äîëæíû âûáðàòü ðîä çàíÿòèé äëÿ äàííîãî ÷åëîâåêà!");
				valid = false;
			}
		}
		if (valid) {
			...
			if (!this->list_citizens->search_registered_citizen_by_all_data(number_apartament, tmp->fio, tmp->birth.year, tmp->birth.month, tmp->birth.day, tmp->target)) {
				...
				this->errorProvider2->SetError(textBox8, "Òàêàÿ çàïèñü óæå ñóùåñòâóåò!");
				this->errorProvider2->SetError(dateTimePicker1, "Òàêàÿ çàïèñü óæå ñóùåñòâóåò!");
				this->errorProvider2->SetError(comboBox1, "Òàêàÿ çàïèñü óæå ñóùåñòâóåò!");
				this->errorProvider2->SetError(groupBox1, "Òàêàÿ çàïèñü óæå ñóùåñòâóåò!");
			}
		}
	}
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		// Ïðîâåðêà ïåðâîãî ïîëÿ
		fio = this->textBox1->Text;
		if (System::String::IsNullOrEmpty(fio)) {
			this->errorProvider1->SetError(this->textBox1, "Òðåáóåòñÿ ÔÈÎ âëàäåëüöà!");
			valid = false;
		}

		// Ïðîâåðêà âòîðîãî ïîëÿ
		try {
			number_apartament = System::Convert::ToInt32(this->textBox2->Text->ToString());
			if (number_apartament <= 0) {
				this->errorProvider1->SetError(this->textBox2, "Íîìåð êâàðòèðû äîëæåí áûòü áîëüøå íóëÿ!");
				valid = false;
			}
			else if (this->list_citizens->search_by_apartament_number(number_apartament) != NULL) {
				this->errorProvider1->SetError(this->textBox2, "Êâàðòèðà óæå çàíÿòà!");
				valid = false;
			}
		}
		catch (...) {
			this->errorProvider1->SetError(this->textBox2, "Íîìåð êâàðòèðû äîëæåí áûòü ÷èñëîâûì!");
			valid = false;
		}

		// Ïðîâåðêà òðåòüåãî ïîëÿ
		try {
			stage = System::Convert::ToInt32(this->textBox3->Text->ToString());
			if (stage <= 0) {
				this->errorProvider1->SetError(this->textBox3, "Ýòàæ äîëæåí áûòü áîëüøå íóëÿ!");
				valid = false;
			}
		}
		catch (...) {
			this->errorProvider1->SetError(this->textBox3, "Ýòàæ äîëæåí áûòü ÷èñëîâûì!");
			valid = false;
		}

		// Ïðîâåðêà ÷åòâåðòîãî ïîëÿ
		try {
			square = System::Convert::ToInt32(this->textBox4->Text->ToString());
			if (square <= 0) {
				this->errorProvider1->SetError(this->textBox4, "Ïëîùàäü êâàðòèðû äîëæåí áûòü áîëüøå íóëÿ!");
				valid = false;
			}
		}
		catch (...) {
			this->errorProvider1->SetError(this->textBox4, "Ïëîùàäü êâàðòèðû äîëæåí áûòü ÷èñëîâûì!");
			valid = false;
		}
		if (valid) {
			...
			System::Windows::Forms::MessageBox::Show("Âû äîáàâèëè âëàäåëüöà êâàðòèðû!", "Óñïåõ", System::Windows::Forms::MessageBoxButtons::OK, System::Windows::Forms::MessageBoxIcon::Information);
			this->textBox1->Clear();
			this->textBox2->Clear();
			this->textBox3->Clear();
			this->textBox4->Clear();
	...
	private: System::Void dataGridView1_CellContentClick(System::Object^ sender, System::Windows::Forms::DataGridViewCellEventArgs^ e) {
		if (e->RowIndex > -1) {
			...
			case 5:
			{
				switch (System::Windows::Forms::MessageBox::Show("Âû óâåðåíû ÷òî õîòèòå óäàëèòü äàííóþ çàïèñü? Çàïèñü áóäåò óòåðÿíà íàâñåãäà", "Ïîäòâåðæäåíèå ïîëüçîâàòåëÿ", System::Windows::Forms::MessageBoxButtons::OKCancel, System::Windows::Forms::MessageBoxIcon::Question))
				{
				case System::Windows::Forms::DialogResult::OK:
				{
					System::Windows::Forms::DataGridViewRow^ row = dataGridView1->Rows[e->RowIndex];
					...
					System::Windows::Forms::MessageBox::Show("Âû óñïåøíî óäàëèëè äàííóþ çàïèñü", "Óñïåõ", System::Windows::Forms::MessageBoxButtons::OK, System::Windows::Forms::MessageBoxIcon::Information);
				}
				default:
					break;
				}
	...
	private: System::Void dateTimePicker1_ValueChanged(System::Object^ sender, System::EventArgs^ e) {
		...
		if (!c) {
			errorProvider2->SetError(this->dateTimePicker1, "Ãðàæäàíèí åù¸ íå ðîäèëñÿ!");
